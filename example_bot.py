...
from maxbot.types import Message, Callback, Update, UpdateType
...
async def handle_text_message(message: Message):
    ...
    session_id = f"{chat_id}:{user_id}" if message.recipient.chat_type in ["chat", "group"] else chat_id
...
async def handle_callback(callback: Callback):
    ...
    bot.answer_callback(
        ...
        notification=f"Button pressed: {payload}"
    )
